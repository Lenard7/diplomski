...
for i in range(len(data)):
    if data[i][0] == 0 and data[i][1] == 0:
        ...
        if flag == 1:
            ...
            plt.plot(X, Y, 'yo', label = "sixth candidate trajectory")
        elif flag == 7:
            plt.plot(X, Y, 'ko', label = "seventh candidate trajectory")
        elif flag == 8:
            plt.plot(X, Y, marker='o', color='brown', linewidth=0, label = "eighth candidate trajectory")
        elif flag == 9:
            plt.plot(X, Y, marker='o', color='orange', linewidth=0, label = "ninth candidate trajectory")
        elif flag == 10:
